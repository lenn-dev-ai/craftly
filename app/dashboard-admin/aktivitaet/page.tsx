...
export default function AktivitaetPage() {
  ...
  const TYPE_CONFIG: Record<string, { color: string; label: string }> = {
    ticket: { color: "#00D4AA", label: "Ticket" },
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          {[
            { label: "Tickets", value: data.totalT, color: "#00D4AA" },
            { label: "Nutzer", value: data.totalU, color: "#8B5CF6" },
            { label: "Angebote", value: data.totalA, color: "#00B4D8" },
          ].map((m, i) => (
            <div key={i} className="px-3 py-2 bg-[#12121a] border border-white/[0.06] rounded-xl text-center">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        <div className={"p-4 border rounded-2xl " + (trend.trend === "up" ? "bg-[#00D4AA]/[0.06] border-[#00D4AA]/20" : trend.trend === "down" ? "bg-[#FF6363]/[0.06] border-[#FF6363]/20" : "bg-[#8B5CF6]/[0.06] border-[#8B5CF6]/20")}>
          <span className={"text-xs font-bold uppercase tracking-wider " + (trend.trend === "up" ? "text-[#00D4AA]" : trend.trend === "down" ? "text-[#FF6363]" : "text-[#8B5CF6]")}>KI Trend-Analyse</span>
          <div className={"text-sm mt-1 " + (trend.trend === "up" ? "text-[#00D4AA]/80" : trend.trend === "down" ? "text-[#FF6363]/80" : "text-[#8B5CF6]/80")}>
            {trend.trend === "up" ? "^ " : trend.trend === "down" ? "v " : "= "}{trend.text}
          </div>
        </div>
        {peak && (
          <div className="p-4 bg-[#F59E0B]/[0.06] border border-[#F59E0B]/20 rounded-2xl">
            <span className="text-xs font-bold text-[#F59E0B] uppercase tracking-wider">KI Peak-Erkennung</span>
            <div className="text-sm text-[#F59E0B]/80 mt-1">Aktivster Tag: {peak}</div>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#12121a] border border-white/[0.06] rounded-2xl p-5 mb-6">
        ...
        <div className="flex items-center gap-6 mt-4 pt-4 border-t border-white/[0.06]">
          <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm bg-[#00D4AA]" /><span className="text-[11px] text-gray-400">Tickets</span></div>
          ...
          <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-sm bg-[#00B4D8]" /><span className="text-[11px] text-gray-400">Angebote</span></div>
        </div>
      </div>
